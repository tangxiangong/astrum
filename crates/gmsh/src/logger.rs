...
pub fn get_last_error() -> GResult<String> {
    ...
    unsafe {
        ...
        let result = match (ierr, error_ptr.is_null()) {
            ...
            (0, true) => Err(GError::InvalidState("Null pointer with success code".to_owned())),
            (_, _) => Err(GError::ApiError("Failed to retrieve error message".to_owned())),
        };
